...
export default function AdminLogin({ onLogin }: Props) {
  // Ubah default state username/email menjadi email resmi Supabase Auth
  ...
  // 👈 2. Fungsi handleSubmit baru menggunakan Supabase Auth
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Login langsung ke backend Supabase
      ...
      if (authError) {
        ...
        // Token autentikasi tersimpan otomatis oleh Supabase Client
        onLogin();
      }
    } catch (err) {
        ...
        >
          <div>
            ...
            <input
              ...
              placeholder="admin@kendalrejo.desa.id"
              required
              className="w-full px-3 py-2.5 bg-background border border-border rounded-xl text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition-colors"
            />
          </div>
